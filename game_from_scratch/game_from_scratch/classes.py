...
#animation
red_exsplotion_image = [pygame.image.load('assets/red_exsplotion1.png'), pygame.image.load('assets/red_exsplotion2.png'), pygame.image.load('assets/red_exsplotion3.png'), pygame.image.load('assets/red_exsplotion4.png')]

json_file_data = json.load(open('stage1.json'))
...
class Playerclass(Shipclass):
    ...
    def __init__(self, x=0, y=0):
        ...
        self.god = False
        self.animaton_framerate = 5 #set framerate (can bechanged to make animation faster or slower)
    ...
    def shoot(self):
        if self.bullet_cooldown_counter == 0:
            ...
            bullet = Playerbulletclass(center_x, center_y, playerbullet, bullet_velocity)
            self.bullets_on_screen.append(bullet)

            #special bullets based on power
            if self.power == 400:
                self.bullets_on_screen.append(Playerbulletclass(center_x-self.getwidth()-arrow_center_x, arrow_y, playerarrow, bullet_velocity))
                self.bullets_on_screen.append(Playerbulletclass(center_x+self.getwidth()+arrow_center_x, arrow_y, playerarrow, bullet_velocity))
                self.bullets_on_screen.append(Playerbulletclass(center_x-self.getwidth()+arrow_center_x, arrow_y, playerarrow, bullet_velocity))
                self.bullets_on_screen.append(Playerbulletclass(center_x+self.getwidth()-arrow_center_x, arrow_y, playerarrow, bullet_velocity))
            elif self.power >= 300:
                self.bullets_on_screen.append(Playerbulletclass(center_x-self.getwidth()+arrow_center_x, arrow_y, playerarrow, bullet_velocity))
                self.bullets_on_screen.append(Playerbulletclass(center_x+self.getwidth()-arrow_center_x, arrow_y, playerarrow, bullet_velocity))
                self.bullets_on_screen.append(Playerbulletclass(center_x, arrow_y, playerarrow, bullet_velocity))
            elif self.power >= 200:
                self.bullets_on_screen.append(Playerbulletclass(center_x-self.getwidth()+arrow_center_x, arrow_y, playerarrow, bullet_velocity))
                self.bullets_on_screen.append(Playerbulletclass(center_x+self.getwidth()-arrow_center_x, arrow_y, playerarrow, bullet_velocity))
            elif self.power >= 100:
...
class Enemyclass(Shipclass):
    ...
    def __init__(self, x, y, health, ship_id, image, ship_velocity, cooldown, worth, amount_of_items_to_drop, amount_of_bullets_shoot, ammo, angle_offset, angle_spread, path):
        super().__init__(x, y)
        #base ship
    ...
    def move(self, enemy_list):
        if self.wait_move > 0:
            ...
            if move_x_done and move_y_done == True:
                self.current_node += 1
                if self.current_node < len(json_file_data['paths'][0][self.path]):
                    if 'die' in json_file_data['paths'][0][self.path][self.current_node]:
                        enemy_list.remove(self)
                    elif 'wait' in json_file_data['paths'][0][self.path][self.current_node]:
                        self.wait_move = json_file_data['paths'][0][self.path][self.current_node]['wait']
                    else:
                        if json_file_data['paths'][0][self.path][self.current_node]['x'] == 'relative':
                            self.goal_x = self.x
                        else:
                            self.goal_x = json_file_data['paths'][0][self.path][self.current_node]['x']

                        if json_file_data['paths'][0][self.path][self.current_node]['y'] == 'relative':
                            self.goal_y = self.y
                        else:
                            self.goal_y = json_file_data['paths'][0][self.path][self.current_node]['y']
                    
                    if 'shoot' in json_file_data['paths'][0][self.path][self.current_node]:
                        self.ready_to_shoot = json_file_data['paths'][0][self.path][self.current_node]['shoot']
    ...
    def die(self, enemy_list, particle_list, point_list, power_list, heart_list, bomb_list):
        particle_list.append(Particleclass(self.x, self.y))
...
class Bossclass(Shipclass):
    ...
    def die(self, enemy_bullets, point_list, power_list, ghost_list):
...
class Particleclass(Classbase):
    ...
    def __init__(self, x, y):
        super().__init__(x, y)
        self.sprite_sheet = red_exsplotion_image
